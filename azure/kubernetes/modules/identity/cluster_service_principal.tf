...
resource "azurerm_role_assignment" "cluster_sp_contributor" {
  ...
  provisioner "local-exec" {
    command = "echo \"pwd=$(pwd)\" && echo \"path.module=${path.module}\" && ${path.module}/../../../../scripts/wait-for-service-principal-contributor-role-to-propagate.sh ${azuread_service_principal.cluster_sp.id}"
  }
}
